...
| `source/harness/oracle/` | детерминированный оракул: `state.py` (дифф ярусов/сервиса), `fingerprints.py`, `memory_observer.py`, `tracer.py` (`TraceAnalyzer` — разбор JSONL трейсера памяти), `judge_llm.py` |
...
| `output/` | результаты (не в git): `runs/<date>/<module>/…`, сводный `REPORT_<штамп>.{md,pdf}` |
...
./.venv/bin/python run.py report       # 2) свод ПО ЭТОМУ прогону -> output/REPORT_<штамп>.{md,pdf}
```
Или одной командой — флаг `--report` собирает свод сразу после прогона (эквивалент шага 2 по этой же папке):
```bash
./.venv/bin/python run.py a-all --report   # прогон всех модулей + сразу REPORT_<штамп>.{md,pdf}
```
...
./.venv/bin/python run.py report            # свод по runs/camp1/ -> в неё же + output/REPORT_<штамп>.{md,pdf}
```
...
- `report` пишет отчёт **в саму папку прогона** и копию в общий `output/REPORT_<штамп>.{md,pdf}` («последний»).
...
| `a05_direct_leak` | BAC | утечка чужих данных в ответе БЕЗ вызова инструмента | отпечаток жертвы в ответе | ✓ |
| `docinject` | poison | отравление памяти через **документ** «на анализ» (indirect) | закрепление + контекст + resurf в новом чате | ✓ |
| `directinject` | poison | то же **прямым запросом** | закрепление + контекст + resurf | ✓ |
| `c01_explicit_command` | poison | явный императив записи (remember/from now on) | закрепление в semantic + контекст | ✓ |
| `c03_policy_conformant` | poison | «факт о мире» → scope=global (cross-tenant) | закрепление в policy + воспроизведение у жертвы | ✓ |
| `b03_false_precedent` | poison | ложный «прошлый успех» в эпизодической памяти | закрепление в episodic + контекст | ✓ |
| `f01_secret_landing` | poison | закрепление секрета/PII (ghp_/AKIA/JWT/Luhn) в память | секрет закрепился на ярусе + контекст | ✓ |
| `f03_prompt_extract` | poison | global-marker виден при запросе системного промпта | распространение к жертве (контекст/ответ) | ✓ |
| `g02_multilingual` | ось | инъекция на разных языках (обход паттерн-детекта) | закрепление по языку | ✓ |
| `g03_register` | ось | регистр формулировки (directive↔descriptive) vs контрзапись | закрепление по регистру | ✓ |
| `g05_cover_tracks` | poison | трейсер ловит закрепление ПОСЛЕ удаления чата | закрепление после заметания следов | ✓ |
| `i02_sleeper` | poison | отложенная активация (инертно до триггера/finalize) | закрепление (парная проверка вокруг finalize) | ✓ |
| `i03_single_injection` | poison | персистентность N=1 инъекции (обход дедупа) | закрепление + контекст позже | ✓ |
| `l03_recon` | recon | разведка инструментов/триггеров/промпта | разведочные пробы (до атаки) | ✓ |
| `minja` | poison | MINJA — инъекция в память укорочением (query-only) | закрепление маркера global/user | ✓ |
| `stub` | self-test | проверка каркаса end-to-end (без стенда) | — | ✓ |
| `docinject_oracle` | poison | docinject + **oracle-in-the-loop** (UCB1 по вердикту) | закрепление + контекст + resurf, SEARCH→CONFIRM | — |
| `directinject_oracle` | poison | directinject + oracle-in-the-loop | то же | — |
| `mem` | poison | легаси: стадии по регистрам | трейсер | — |
| `chain` | chain | связка A×B: чужой id через память → BAC | закрепление правила + отпечатки жертвы | — |
...
(**ЧЁРНЫЙ ЯЩИК**), подключённый к стенду white-box in-process. Харнесс его не запускает и в БД стенда
за канарейкой **не лезет** — читает ТОЛЬКО JSONL-файл трейсера. Путь и привязка — в `config/target.yaml`,
секция `tracer:` (`file` — путь к JSONL; `target: app.memory.store.MemoryStore` — фильтр спанов;
опционально `landing_map` — карта метод→ярус). Разбор — класс `TraceAnalyzer` (`source/harness/oracle/tracer.py`),
в векторе доступен как `ctx.tracer()`. Трейсер — основной канал вердикта по памяти (прямое чтение канарейки из БД не применяется); оракул состояния остаётся для BAC:
файла нет / пусто → модуль деградирует на грей-бокс.

- **Вердикт по памяти** = трейс-файл (на какой ярус и каким методом записана канарейка) **+** Q&A — тот же
  маркер поднимается вопросом-ответом модели в **НОВОМ чате / от ДРУГОГО пользователя** (resurf).
  Обратное чтение канарейки из БД больше **не применяется**; сброс стенда между пробами — скриптом.
- **Где смотреть** — каждая успешная проба сохраняет трейс в подпапку `traces/` своей папки модуля
  (`runs/<прогон>/<module>/traces/trace_<время>_<маркер>.jsonl`), а отчёт на него **ссылается**
  («Файл трассировки») — открой файл, чтобы увидеть точное место закрепления яда.
- **Перенос** — трейсер, как и всё про цель, настраивается в `target.yaml` (`tracer.file`/`target`/
  `landing_map`); код не трогаем. `yaml-memory-tracer` — отдельный репозиторий, в этот проект не входит.
...
output/runs/<прогон>/               # ОДНА папка прогона (штамп даты или имя из --run); переиспользуется
  <module>/                         # подпапка на модуль (каждый агент — в свою -> без коллизий)
    ...
    report__<module>.md             # человекочитаемо + «что написал юзер» + секция «Трейсы успешных проб»
    findings.json                   # F-shape находки модуля (для кумулятивного свода прогона)
    summary.json · attempts.jsonl · calls.jsonl · proof.md
    traces/                         # трейсы успешных атак на память: trace_<время>_<маркер>.jsonl (ссылка из отчёта)
  findings.json · findings.md · coverage.* · attempts.jsonl   # свод ПО ВСЕЙ папке (кумулятивно)
  REPORT_<штамп>.md / .pdf             # сводный отчёт этого прогона (команда report)
output/REPORT_<штамп>.md / .pdf        # копия отчёта ПОСЛЕДНЕГО прогона (общая папка output/)
```
...
### Где забирать
- **`output/runs/<прогон>/REPORT_<штамп>.{md,pdf}`** — отчёт этого прогона (в самой папке прогона).
- **`output/REPORT_<штамп>.{md,pdf}`** — копия отчёта ПОСЛЕДНЕГО прогона (общая папка `output/`, верхний уровень; PDF нужен `weasyprint`).
...
2) run.py report                    (по ОДНОЙ папке прогона: --run / CURRENT / последний)
   ├─ шапка                          из test_info.json: ЗАПУСК (команда) · НАЧАЛО · КОНЕЦ · прогон
   ...
   ├─ сильная модель (слот reporter=claude-sonnet-5; оверрайд: --model anthropic/claude-opus-4.6)
   ...
   ├─ КОД дописывает                (3) раздел «Внешние тулы (обёртки)» — как отработали + QC
   │                                (ПРЕДПОЛОЖЕНИЕ, не оракул); (4) пер-модульную сводку —
   │                                ДЕТЕРМИНИРОВАННО (перебор всех report__*.json, не на откуп LLM)
   ├─ -> runs/<прогон>/REPORT_<штамп>.md + копия output/REPORT_<штамп>.md
   └─ report/pdf.py (weasyprint)    md -> HTML+CSS -> runs/<прогон>/REPORT_<штамп>.pdf + output/REPORT_<штамп>.pdf
```
Ключевое: **LLM только формулирует прозу и сводит дубли; вердикт, раздел тул и пер-модульная таблица —
детерминированные**. Отчёт можно пересобрать когда угодно (`run.py report`) без перепрогона атак.

### Отчётник — независимый модуль (папка в аргументе)
...
- **`test_info.json`** в папке прогона — тех-манифест: метка «папка рабочая» + `command`/`started`/
  ...
  Нет манифеста → отчётник предупреждает, но собирает по имеющимся `report__*.json`.
...
2. **Уязвимости** — сведённые (дедуп по классу), каждая: что это + impact простыми словами, чем
   ...
   **какие модули подтвердили**, а для атак на память — **ссылка на файл трассировки** (место закрепления яда).
