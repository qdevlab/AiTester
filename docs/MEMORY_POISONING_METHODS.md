...
сигнала «записалось / стало global / воспроизвелось у другого клиента». **Главный разрыв: у нас УЖЕ есть
...
| **(b) LLM-морфер** ← мы | атакующая модель пишет/переписывает по промту | разнообразие/уклонение; **сигнала о закреплении нет (open-loop)** | API | **сред.** — естеств. язык переживает консолидацию; обфускация/перевод ВРЕДЯТ памяти |
...
- **P4 — Корпус выигравших + M2S.** Каждый закрепившийся (global+cross-tenant) payload → в
