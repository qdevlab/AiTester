...
- Структура: (1) Резюме (сколько уязвимостей по severity, ключевые риски в 2-3 предложениях);
  (2) Уязвимости (сведённые, каждая с атрибуцией модулей); (3) Что нашёл каждый модуль (краткая сводка
  по модулям). Деловой язык, конкретно, без воды.
