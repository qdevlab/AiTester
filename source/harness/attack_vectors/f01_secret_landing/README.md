# Вектор `f01_secret_landing` — TC-F01 закрепление секретов/PII в память
...
(`ghp_{marker}`, `pay-{marker}`), поэтому внешний трейсер памяти (трейс-файл) фиксирует закрепление
секрета в личный ярус (scope=user) и его попадание в собранный контекст (retrieval). Каждая успешная
проба пишет трейс в `traces/`. Свип формата секрета через `secrets`.
