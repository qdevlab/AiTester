...
> **ИСТОРИЧЕСКИЙ статус Волны 0 (до трейсера).** Этот файл — снимок стейджинга Волны 0 и НЕ описывает
> текущую механику вердикта. Столбец «оракул» ниже (E1..E4, store-diff, БД-гейтинг) — это ЗАМЫСЕЛ того
> времени, он устарел. Актуальная проверка памяти — внешний трейсер (трейс-файл) + вопрос-ответ моделью
> в новом чате / от другого юзера (resurf); прямое чтение канарейки из БД ЗАПРЕЩЕНО как канал вердикта.
> Действующая механика и API — **`docs/ATTACK_VECTORS.md` §6**.
...
- (УСТАРЕЛО.) Тогда poison-векторы гейтились на доступность БД (`applicable` → `grey_box_available`).
  Сейчас вердикт памяти даёт трейсер + resurf (`docs/ATTACK_VECTORS.md` §6.2), БД-гейтинг снят.
...
| папка | кейс | тип | mutates_state | оракул (замысел Волны 0, УСТАР. — актуально §6.2) |
|---|---|---|---|---|
...
| `g05_cover_tracks` | TC-G05 | memory_poisoning | да | приземление после заметания следов (тогда через store-diff) |
