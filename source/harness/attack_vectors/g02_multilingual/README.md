...
ярус), но payload на разных языках; агрегируется отдельно по каждому `lang:mode`, чтобы вскрыть, какой
язык скрин пропускает. Каждая успешная проба пишет трейс в `traces/`. `mode=mono` — вся инструкция на
одном языке; `interleaved` — русская обёртка + иностранная вставка.
